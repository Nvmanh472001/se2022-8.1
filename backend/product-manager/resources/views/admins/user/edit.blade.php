            ...
            <div class="container-fluid">
                <div class="row">
                    ...
                    <div class="col-md-12 justify-content-md-center">
                        <form action="{{ route('users.update', ['id' => $users->id]) }}" enctype="multipart/form-data"
                            method="post">
                            @csrf
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input name="name" placeholder="Nhập họ và tên"
                                        value="@if (!empty(old('name'))) {{ old('name') }}@else{{ $users->name }} @endif"
                                        type="text" class="form-control" required>
                                </div>
                                <div class="form-group">
                                    <label class="form-label">Số điện thoại</label>
                                    <input name="phone" type="phone"
                                        value="@if (!empty(old('phone'))) {{ old('phone') }}@else{{ $users->phone }} @endif"
                                        type="text" class="form-control"placeholder="Nhập số điện thoại" required>
                                    @error('phone')
                                ...
                                <div class="form-group">
                                    ...
                                    <input name="email" type="email"
                                        value="@if (!empty(old('email'))) {{ old('email') }}@else{{ $users->email }} @endif"
                                        type="text" class="form-control" placeholder="Nhập email" required>
                                    @error('email')
                                        <span style="color: red">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="form-group">
                                    ...
                                    <input value='' id="myInput" name="password" type="password"
                                ...
                                <div class="form-group">
                                    ...
                                    <input id="myInputs" value='' type="password" class="form-control"
                                        name="res_password" placeholder="Nhập lại mật khẩu">
                                    @error('res_password')
                                        <span style="color: red">{{ $message }}</span>
                                    @enderror
                                ...
                                <div style="margin-top : 15px" class="form-group">
                                    ...
                                    <select style="width: 250px;" multiple class="form-select js-states select2_role"
                                        id="selectid" value="{{ old('detail') }}" name="role_id[]">
                                        <option></option>
